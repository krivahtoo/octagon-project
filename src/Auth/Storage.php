...
/**
 * Custom version of OAuth2\Storage\Pdo
 */
  ...
  OpenIDAuthorizationCodeInterface
{
   ...
   */
  public function __construct(PDO $connection, $config = array())
  {
    $this->db = $connection;
   ...
   * @return bool
   */
  public function checkUserCredentials($phone, $password)
  {
  ...
  {
    $stmt = $this->db->prepare($sql = sprintf('SELECT * from %s where phone=:phone', $this->config['user_table']));
    ...
    // the default behavior is to use "phone" as the user_id
   ...
   * @return bool
   */
  public function setUser($phone, $password, $firstName = null, $lastName = null)
  {
    ...
    if ($this->getUser($phone)) {
      $stmt = $this->db->prepare($sql = sprintf('UPDATE %s SET password=:password, first_name=:firstName, last_name=:lastName where phone=:phone', $this->config['user_table']));
    } else {
      $stmt = $this->db->prepare(sprintf('INSERT INTO %s (phone, password, first_name, last_name) VALUES (:phone, :password, :firstName, :lastName)', $this->config['user_table']));
    }

    return $stmt->execute(compact('phone', 'password', 'firstName', 'lastName'));
  }
